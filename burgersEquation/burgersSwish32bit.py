...
try: # load saved network if possible
    checkpoint = torch.load('burgersSwish1.pth')
    ...
    try: # load initial network
        checkpoint = torch.load('burgersSwishInitialNetwork.pth')
        network = checkpoint['network']
        trainData = checkpoint['trainData']
        ...
        checkpoint = {'network': network,
                       'trainData' : trainData}
        torch.save(checkpoint, 'burgersSwishInitialNetwork.pth')
...
while epoch < 100000:
    ...
    torch.save(checkpoint, 'burgersSwish1.pth')
    print("model saved")
