...
def trainBoth(network, lossFn, optimiser, scheduler, loader, numEpochs):
...
try: # load saved network if possible
    checkpoint = torch.load('burgersFinalExperiment.pth')
...
while epoch < 50000:
    ...
    torch.save(checkpoint, 'burgersFinalExperiment.pth')
    print("model saved")
...
lambda2List = []
iterations = 0
numEpochs = 50000 # number of epochs to train each iteration
while iterations < 5:
    newLoss, newLambda1, newLambda2 = trainBoth(network, lossFn, optimiser, scheduler, trainLoader, numEpochs)
    uLosses.extend(newLoss)
