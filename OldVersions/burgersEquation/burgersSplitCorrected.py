...
class Fitter(torch.nn.Module):
    ...
    def forward(self, input):
        hidden = ReTanh(self.fc1(input))
        for i in range(len(self.fcs)):
            hidden = ReTanh(self.fcs[i](hidden))
...
def ReTanh(x):
    ...
    """
    return (1. + torch.sign(x)) * torch.tanh(x)
...
try: # load saved network if possible
    checkpoint = torch.load('burgersSplitReTanh.pth')
...
while epoch < 30000:
    ...
    torch.save(checkpoint, 'burgersSplitReTanh.pth')
    print("model saved")
