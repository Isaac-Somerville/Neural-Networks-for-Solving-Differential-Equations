...
def plotNetwork(network, X, T, XT, u_exact, epoch):
    ...
    u_exact = torch.tensor(u_exact).float().reshape(X.shape[0],X.shape[1])
    ...
    # Plot exact solution
    # ax.plot_surface(X,T,u_exact,rstride=1, cstride=1,
                # cmap='plasma', edgecolor='none')
    # ax.scatter(X,T,u_exact, label = 'Exact Solution')
    # ax.legend()

    ax.set_title("Burger's Equation: Network's Output", fontsize = 16)
    # ax.set_title("Burger's Equation: Exact Solution", fontsize = 16)
    plt.show()
    return
...
try: # load saved network if possible
    checkpoint = torch.load('burgersAdam2.pth')
...
plotNetwork(network, X, T, XT, u_exact, 100000)
plt.semilogy(losses[:100000])
plt.xlabel("Epochs", fontsize = 16)
plt.ylabel("Cost", fontsize = 16)
plt.title("Burger's Equation Training Cost", fontsize = 16)
plt.show()

plt.plot(lambda1s[:100000], label = "\u03BB Value")
plt.xlabel("Epochs", fontsize = 16)
# plt.ylabel("\u03BB\u2081 Value", fontsize = 16)
# plt.title("Burger's Equation \u03BB\u2081 Values", fontsize = 16)

plt.plot(lambda2s[:100000], label = "\u03BD Value")
plt.xlabel("Epochs", fontsize = 16)
plt.ylabel("Parameter Value", fontsize = 16)
plt.legend(loc = "upper right", fontsize = 16)
plt.title("Burger's Equation \u03BB and \u03BD Values", fontsize = 16)
plt.show()
...
while iterations < 5:
    ...
    torch.save(checkpoint, 'burgersAdam2.pth')
