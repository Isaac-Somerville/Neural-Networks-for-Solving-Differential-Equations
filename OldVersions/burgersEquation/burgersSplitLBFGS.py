...
try: # load saved network if possible
    checkpoint = torch.load('burgersSplitLBFGS.pth')
    ...
    optimiser  = torch.optim.LBFGS(network.parameters(), lr =1e-3, max_iter = 50000, max_eval = 50000)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimiser, 
        factor=0.5, 
        patience=2000, 
...
while epoch < 5000:
    ...
    #     'trainData': trainData
    #     }
    # torch.save(checkpoint, 'burgersSplitLBFGS.pth')
    # print("model saved")
