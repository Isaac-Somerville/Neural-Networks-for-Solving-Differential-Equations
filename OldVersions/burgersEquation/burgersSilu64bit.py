...
class Fitter(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        hidden = swish(self.fc1(input))
        for i in range(len(self.fcs)):
            hidden = swish(self.fcs[i](hidden))
...
try: # load saved network if possible
    checkpoint = torch.load('burgersSilu64bit2.pth')
    ...
    # checkpoint = torch.load('burgersSwish64bit.pth')
    # trainData = checkpoint['trainData']
...
while epoch < 100000:
    ...
    torch.save(checkpoint, 'burgersSilu64bit2.pth')
    print("model saved")


lambda1     = torch.tensor(torch.rand(1), requires_grad = True) 
lambda2     = torch.tensor(torch.rand(1), requires_grad = True)
optimiser   = torch.optim.Adam([{"params": lambda1, 'lr' : 1e-3}, {"params": lambda2, 'lr' : 1e-3}])
