...
try: # load saved network if possible
    checkpoint = torch.load('burgersSilu32bit2.pth')
    ...
    try: # load initial network
        checkpoint = torch.load('burgersSiluInitialNetwork2.pth')
        network = checkpoint['network']
        trainData = checkpoint['trainData']
        ...
        checkpoint = {'network': network,
                       'trainData' : trainData}
        torch.save(checkpoint, 'burgersSiluInitialNetwork2.pth')
...
while epoch < 100000:
    ...
    torch.save(checkpoint, 'burgersSilu32bit2.pth')
    print("model saved")
