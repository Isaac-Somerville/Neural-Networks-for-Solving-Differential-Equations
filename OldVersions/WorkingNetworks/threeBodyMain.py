...
def plotNetwork(network, mu, batchNum,
    ...
    if batchNum % 500000 == 0:
        plt.savefig(str(batchNum) + '.png')
        print("figure saved")
    plt.show()
...
try: # load model
    checkpoint = torch.load('threeBodyShorterInterval2.pth')
    ...
    try: # load backup of model
        checkpoint = torch.load('threeBodyShorterInterval2Backup.pth')
        batchNum = checkpoint['batchNum']
        network = checkpoint['network']
        optimiser = checkpoint['optimiser']
        scheduler = checkpoint['scheduler']
...
#     if batchNum != 0:
    ...
    if batchNum % 50000 == 0 and progressMade == True:
        ...
        torch.save(checkpoint, 'threeBodyShorterInterval2.pth')
        print("model saved")
    if batchNum % 70000 == 0 and progressMade == True:
        ...
        torch.save(checkpoint, 'threeBodyShorterInterval2Backup.pth')
